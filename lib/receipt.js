...
/**
 * Kontrollens beteckning på kvittot (KRAV-19): numret först, namnet som
 * suffix inom parentes – "50 (Radio 1)".
 *
 * Numret är det löparen har på banbeskrivningen och det enda som går att
 * jämföra mot skärmen i skogen. Namnet är arrangörens egen etikett, sätts
 * bara på de kontroller MeOS namngett (i praktiken radiokontrollerna) och
 * duger inte ensamt för att peka ut en kontroll.
 */
function controlLabel(cmp, code) {
  const namn = cmp.controls[code]?.name || '';
  // Före KRAV-19 skrev lib/mop.js "Kontroll <nr>" som namn på en namnlös
  // kontroll. Platshållaren ligger kvar i redan sparad data och ser ut som
  // ett namn utan att vara det – utan det här hade den blivit
  // "77 (Kontroll 77)" på varje kvitto från en tävling som sparats tidigare.
  if (!namn || namn === `Kontroll ${code}`) return String(code);
  return `${code} (${namn})`;
}
