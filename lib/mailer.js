...
export function createMailer({ transport, from, replyTo = '' }) {
  ...
  return {
    ...
    async sendReceipt({ to, receipt }) {
      ...
      // Samma uppgifter och samma ordning som kvittosidan och PDF:en. En
      // handplockad delmängd glider isär från dem: sammanfattningen läses i
      // förhandsvisningen på låsskärmen, och sa förut "Status: Godkänd" utan
      // förbehåll även när resultatet var preliminärt.
      const summary = [
        `${name} – ${cmp}`,
        res.time ? `Tid: ${res.time}` : '',
        res.statusText ? `Status: ${res.statusText}` : '',
        res.preliminary ? 'Preliminärt resultat – ej fastställt' : '',
        res.place
          ? `Placering: ${res.place} av ${res.finished} i mål`
          : res.prelPlace
            ? `Prel. placering: ${res.prelPlace} av ${res.finished} i mål`
            : '',
        // Stafett: sträcktiden ensam säger inte hur laget ligger till (KRAV-3)
        res.teamTime ? `Lagets tid: ${res.teamTime}` : '',
        // KRAV-25: bara totalen. Vilken kontroll tiden gick på står i den
        // bifogade PDF:en – en lista med kontroller i förhandsvisningen på
        // låsskärmen säger ingenting utan tabellen omkring sig.
        receipt.timeLoss?.total ? `Total tidsförlust: ${receipt.timeLoss.total}` : '',
      ].filter(Boolean);
