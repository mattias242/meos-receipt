...
function processControl(cmp, d) {
  const id = int(d.id);
  if (!id) return;
  // Namnlös kontroll sparas utan namn, inte med platshållaren "Kontroll <id>":
  // kvittot visar då bara numret (KRAV-19), och en platshållare hade blivit
  // suffixet i "77 (Kontroll 77)".
  cmp.controls[id] = { name: text(d) };
}
