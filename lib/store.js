...
export function createStore({
  ...
  // Sparningen skriver till en tmp-fil och byter sedan namn. Avbryts processen
  // däremellan blir tmp-filen liggande med hela deltagarfältet, och skrivs bara
  // över om tjänsten sparar igen – vilket den inte gör efter tävlingens sista
  // sändning. Den är per definition ofullständig, så den städas här.
  if (file && fs.existsSync(`${file}.tmp`)) {
    try {
      fs.unlinkSync(`${file}.tmp`);
      console.log('Städade bort en ofullständig sparfil från en avbruten körning.');
    } catch (err) {
  ...
  function purgeUndanlagdaFiler(maxAge) {
    ...
    let filer;
    try {
      filer = fs.readdirSync(dataDir);
    } catch {
      return; // katalogen kan saknas, t.ex. innan första sparningen
    }
    for (const namn of filer) {
      const m = /^competitions\.json\.trasig-(\d+)$/.exec(namn);
      if (!m) continue;
      const stämplad = Number(m[1]);
      if (!Number.isFinite(stämplad) || now() - stämplad <= maxAge) continue;
      try {
        fs.unlinkSync(path.join(dataDir, namn));
