 ...
 *
 * Bomkolumnen (KRAV-25) fick inte plats utan att ta av de andra – remsan
 * använde redan alla 45 tecknen. Bredderna är mätta mot värdenas verkliga
 * maxbredd och inte gissade: klockan är alltid 8 tecken ("10:06:07"), och
 * sträck-, total- och bomtid blir som längst 7 ("1:02:03"). Var och en har
 * alltså ett tecken luft till kolumnen före, vilket är vad KRAV-17 kräver.
 *
 * Kontrollkolumnen betalar det femte tecknet. "50 (Radio 1)" är 12 tecken och
 * ryms fortfarande; blir det trängre är det namnet som faller bort, aldrig
 * koden eller SAKNAS/EXTRA (KRAV-19).
 */
const COL_CTRL_BOM = 13;
const COL_CTRL = 14;
const COL_TIME_BOM = 8;
const COL_TIME = 10;
const COL_LOSS = 7;
const COL_CLOCK_BOM = 9;
const COL_CLOCK = COLS - COL_CTRL - 2 * COL_TIME;

/** Kolumnbredderna med respektive utan bomkolumn – båda summerar till COLS. */
function tabellMatt(medBom) {
  return medBom
    ? { ctrl: COL_CTRL_BOM, time: COL_TIME_BOM, clock: COL_CLOCK_BOM, loss: COL_LOSS }
    : { ctrl: COL_CTRL, time: COL_TIME, clock: COL_CLOCK, loss: 0 };
}
...
/**
 * Kontrollbeteckningen i den 14 tecken breda kontrollkolumnen (KRAV-19).
 *
 * "150 (Radio 1) SAKNAS" är 20 tecken och skulle klippas mitt i parentesen,
 * så att både numret och beskedet om vad som hänt gick förlorade. Ryms inte
 * allt är det namnet som får falla bort i sin helhet: numret pekar ut
 * kontrollen och markören säger vad som hände med den.
 */
function fitControl(name, mark, width = COL_CTRL) {
  if ((name + mark).length <= width) return name + mark;
  const utanNamn = name.replace(/ \(.*\)$/, '');
  return pad(utanNamn + mark, width);
}
...
export function receiptLines(receipt) {
  const { competition: c, runner: r, result: res, splits = [], notes = {} } = receipt;
  const lines = [];
  ...
  if (splits.length) {
    // Bomkolumnen ritas bara när analysen kunnat göras (KRAV-25). En kolumn
    // som alltid stod där hade tagit ett tecken från kontrollnamnet på varje
    // kvitto, och i skarp data har de flesta klasser för få löpare.
    const medBom = !!receipt.timeLoss?.available;
    const kol = tabellMatt(medBom);

    add(RULE);
    add(
      pad('Kontroll', kol.ctrl) +
        padNum('Sträcka', kol.time) +
        padNum('Total', kol.time) +
        padNum('Klocka', kol.clock) +
        (medBom ? padNum('Bom', kol.loss) : ''),
      { bold: true }
    );
    for (const s of splits) {
      ...
      // raden från en kontroll löparen aldrig stämplade, och fotnoten nedan
      // säger varför.
      const tom = s.unreliable ? '*' : '-';
      add(
        pad(fitControl(s.name, mark, kol.ctrl), kol.ctrl) +
          padNum(s.leg || tom, kol.time) +
          padNum(s.elapsed || tom, kol.time) +
          padNum(s.clock || tom, kol.clock) +
          // Tom cell och inte streck: en sträcka utan bomtid saknar inte data,
          // den ligger på klassens nivå. Strecket betyder "ingen tid" i de
          // andra kolumnerna och skulle säga fel sak här.
          (medBom ? padNum(s.loss || '', kol.loss) : '')
      );
    }
    if (notes.unreliableTimes) add(`* ${notes.unreliableTimes}`);
    if (notes.extraPunches) add(notes.extraPunches);
    if (notes.timeLoss) add(notes.timeLoss);
    if (receipt.timeLoss?.total) add(`Total tidsförlust: ${receipt.timeLoss.total}`);
  }
