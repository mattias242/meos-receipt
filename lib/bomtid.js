 ...
 *
 * `mina` är löparens sträckor med giltig tid, `banan` alla sträckor i hennes
 * bana (även de utan tid) – MeOS `bestTime` summerar baslinjen över hela banan
 * och inte bara över det löparen hann med.
 */
export function bomtider({ mina, banan, baslinjer: bas }) {
  const strackor = mina || [];
  ...
  const bestTime = (banan || []).reduce((a, s) => a + (bas.get(s.nyckel)?.bas ?? 0), 0);

  if (!(baseSum > 0) || !(resSum > 0) || !(bestTime > 0)) return ut;

  const första = pass(res, b, resSum, baseSum, bestTime);
  ...
  const andra = bomfri > 0 ? pass(res, b, bomfri, baseSum, bestTime) : första;
...
export function bomanalys({ klassensStrackor, mina, banan }) {
  ...
  return {
    available: true,
    orsak: null,
    bommar: bomtider({ mina: strackor, banan, baslinjer: baslinjer(klassensStrackor) }),
  };
}
