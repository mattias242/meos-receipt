 ...
 */
/**
 * Varnar när en resultatfil troligen skickats till fel tävling.
 *
 * Sätts MeOS och uppladdningsskriptet till olika tävlings-id hamnar inflödena
 * i skilda tävlingar. Båda svarar OK, men löparen får sin placering räknad på
 * bara den delmängd av fältet som finns i resultatfilen – ett fel som annars
 * inte syns någonstans. Att ingen bricka alls matchar, medan samma brickor
 * finns i en annan tävling, är ett starkt tecken på just det.
 */
function varnaOmFelTavling(store, cid, cmp, results, matchade) {
  if (matchade > 0 || results.length === 0) return;

  // Att brickorna finns i en annan tävling räcker inte som tecken – samma
  // löpare deltar i flera tävlingar med samma bricka. Det som skiljer
  // felkonfigurationen från en ny tävling är att filen skapat en dubblett:
  // en annan tävling har samma namn och datum, och brickorna finns där.
  const kort = new Set(results.map((r) => r.card).filter((c) => c > 0));
  for (const [annanCid, annan] of Object.entries(store.competitions)) {
    if (Number(annanCid) === Number(cid)) continue;
    const sammaTavling =
      annan.info.name && annan.info.name === cmp.info.name && annan.info.date === cmp.info.date;
    if (!sammaTavling) continue;
    const traffar = Object.values(annan.competitors).filter((c) => kort.has(c.card)).length;
    if (traffar > 0) {
      console.warn(
        `Resultatfilen skickades till tävling ${cid}, men ingen av dess ${results.length} ` +
          `löpare matchade där. Tävling ${annanCid} har samma namn och datum och ` +
          `innehåller ${traffar} av brickorna – kontrollera att MeOS och ` +
          'uppladdningsprogrammet använder samma tävlings-id.'
      );
      return;
    }
  }
...
export function applyIof(store, cid, xml) {
  ...
  for (const r of results) {
    ...
    if (id !== null && !cmp.competitors[id].fromIof) matchade++;
  ...
  varnaOmFelTavling(store, cid, cmp, results, matchade);
  store.touch(cid);
}
