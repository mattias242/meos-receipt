...
2. Kör uppladdningsprogrammet på MeOS-datorn så laddas filen upp till
   ...
   **Paketeringen till USB-minne utgår** (2026-08-29): integrationen sker
   numera direkt i MeOS automat, så inget byggs och bärs över till
   MeOS-datorn. Uppladdningsskripten ovan står kvar i `tools/` som reserv och
   testas av `test/uppladdning.test.js`. Se KRAV-11 i `docs/KRAV.md`.
...
| `VARDNAMN_TAVLINGAR` | — | Värdnamn bundna till en bestämd tävling (KRAV-20), t.ex. `kvitto.klubben.se=26082002`; flera skiljs med komma. Ett anrop på `https://kvitto.klubben.se/` skickas då vidare (302) till `/t/26082002`, så att arrangören kan trycka klubbens egen adress i PM i stället för tjänstens domän med ett tävlings-id i sig. Löparen blir kvar på klubbens värdnamn hela vägen. `/t/<id>` fungerar oförändrat för alla värdnamn. Pekas om inför varje arrangemang med `tools/byt-tavling.sh <tävlings-id>`, som skriver om raden, startar om containern och kontrollerar att bindningen tog. Kräver att värdnamnet också pekar hit i DNS och har en vhost i proxyn, se `deploy/DRIFTSATTNING.md`. |
