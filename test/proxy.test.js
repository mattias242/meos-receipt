...
test(
  'bakom en proxy räknas löparen, inte proxyn',
  { concurrency: true },
  ...
  }, { ...GRUND_OPTS, trustProxy: 1 })
);

test(
  'utan proxy går headern inte att sätta själv',
  { concurrency: true },
  withServer(async ({ base }) => {
...
test(
  'med proxy går bara det hopp tjänsten litar på att ändra',
  { concurrency: true },
  withServer(async ({ base }) => {
