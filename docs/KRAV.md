...
| KRAV-8 | Inläst tävlingsdata överlever en omstart av tjänsten. Går den sparade filen inte att läsa startar tjänsten tom, men den oläsbara filen sparas undan i stället för att skrivas över, så att innehållet kan räddas manuellt. | `features/persistens.feature` |
...
| KRAV-13 | Tjänsten driftsätts på internet. Löparna når kvittosidan via mobildata; tävlingsdatorn skickar data över internet (MOP via Onlineresultat samt resultatfiler via uppladdningsprogrammet). Ingen del av lösningen får förutsätta arenanät eller wifi. Eftersom skrivändpunkterna därmed är publika krävs `MEOS_PASSWORD` för att tjänsten ska starta – utan lösenord kontrolleras ingen `pwd`-header, och en enda `MOPComplete` från vem som helst ersätter tävlingen mitt under loppet. `ALLOW_NO_PASSWORD=1` kör medvetet öppet, för reservspåret utan internet (KRAV-12). | Uppfylls av arkitekturen: `POST /meos`/`POST /iof` över HTTPS (features/mop-mottagning.feature, features/resultatfiler.feature) + deployment (Dockerfile, `fly.toml`) |
| KRAV-14 | Tävlingsdata gallras automatiskt när den inte uppdaterats på 90 dagar (konfigurerbart via `RETENTION_DAYS`; `0` stänger av gallringen). Gallringen sker vid start och därefter en gång per dygn, och slår igenom på den persisterade filen. Åldern räknas från senast mottagna data, med tävlingsdatumet som reserv. Undanlagda datafiler (KRAV-8) gallras enligt samma regel – de innehåller hela deltagarfältet, och utan det blir en kopia av personuppgifterna kvar för alltid. | `features/gallring.feature` |
